...
class UsersController {
    async create(request, response) {
        ...
        if (name.length < 3) {
            throw new AppError('Error: Please enter a valid name!');
        }

        if (!email.includes("@", ".") || !email.includes(".")) {
            throw new AppError('Error: Please enter a valid email!');
        }

        if (password.length < 6) {
            throw new AppError('Error: Password must be at least 6 digits long!');
        }
